  ...
  for (var rIndex252 = 0; rIndex252 < rListLen252; rIndex252++) {
    ...
    if (rData252.lane && soy.$$getMapKeys(opt_data.kinds).length > 1) {
      if (opt_data.assignment.template.model.facs.is_ab) {
        var qList395 = soy.$$getMapKeys(opt_data.kinds[rData252.lane.kind].conditions);
        var qListLen395 = qList395.length;
        for (var qIndex395 = 0; qIndex395 < qListLen395; qIndex395++) {
          var qData395 = qList395[qIndex395];
          output.append((opt_data.assignment.template.facs_kinds[rData252.lane.kind].conditions[qData395].identity[rData252.cell_treatment.identifier]) ? '<input name="SQ_' + soy.$$escapeHtml(rData252.cell_treatment.id) + '_' + ((rData252.kind == 'existing') ? soy.$$escapeHtml(rData252.lane.id) : '') + '" class="scb_f_microscopy_select_conditions" facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\' type="radio" value="$q" cell_treatment_id=\'' + soy.$$escapeHtml(rData252.cell_treatment.id) + '\' lane_kind="' + soy.$$escapeHtml(rData252.kind) + '" lane_id="' + ((rData252.kind == 'existing') ? soy.$$escapeHtml(rData252.lane.id) : '') + '"' + ((rData252.is_sample_enabled) ? 'checked="checked"' : '') + '/><span class="scb_s_western_blot_choose_gel_type_input_text ">' + soy.$$escapeHtml(opt_data.assignment.template.facs_kinds[rData252.lane.kind].conditions[qData395].name) + '</span>' : '');
        }
      } else {
  ...
  if (soy.$$getMapKeys(opt_data.kinds).length == 1) {
    ...
    if (opt_data.assignment.template.model.facs.is_ab) {
      if (opt_data.lane.kind == 'existing') {
        var kList590 = soy.$$getMapKeys(opt_data.kinds);
        var kListLen590 = kList590.length;
        for (var kIndex590 = 0; kIndex590 < kListLen590; kIndex590++) {
          var kData590 = kList590[kIndex590];
          output.append((opt_data.live == 'live' && opt_data.lane.has_live && opt_data.kinds[kData590]['Live'][opt_data.cell_treatment.id]) ? '<option value=\'' + soy.$$escapeHtml(kData590) + '\' role=\'option\'' + ((opt_data.lane.lane.kind == kData590) ? 'selected="selected"' : '') + '>' + soy.$$escapeHtml(opt_data.kinds[kData590].name) + '</option>' : '', (opt_data.live == 'fixed' && opt_data.lane.has_fixed && opt_data.kinds[kData590]['Fixed'][opt_data.cell_treatment.id]) ? '<option value=\'' + soy.$$escapeHtml(kData590) + '\' role=\'option\'' + ((opt_data.lane.lane.kind == kData590) ? 'selected="selected"' : '') + '>' + soy.$$escapeHtml(opt_data.kinds[kData590].name) + '</option>' : '');
        }
      } else {
        ...
        if (opt_data.live == 'live' && opt_data.lane.has_live && opt_data.lane.lysate_types['Live']) {
          ...
          for (var kIndex620 = 0; kIndex620 < kListLen620; kIndex620++) {
            ...
            output.append('<option value=\'', soy.$$escapeHtml(kData620), '\' role=\'option\'', (opt_data.lane.lane && opt_data.lane.lane.kind == kData620) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData620].name), '</option>');
          }
        }
        if (opt_data.live == 'fixed' && opt_data.lane.has_fixed && opt_data.lane.lysate_types['Fixed']) {
          ...
          for (var kIndex633 = 0; kIndex633 < kListLen633; kIndex633++) {
            ...
            output.append('<option value=\'', soy.$$escapeHtml(kData633), '\' role=\'option\'', (opt_data.lane.lane && opt_data.lane.lane.kind == kData633) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData633].name), '</option>');
          }
        }
      }
    } else {
      if (opt_data.lane.kind == 'existing') {
        var kList647 = soy.$$getMapKeys(opt_data.kinds);
        var kListLen647 = kList647.length;
        for (var kIndex647 = 0; kIndex647 < kListLen647; kIndex647++) {
          var kData647 = kList647[kIndex647];
          output.append('<option value=\'', soy.$$escapeHtml(kData647), '\' role=\'option\'', (opt_data.lane.lane.kind == kData647) ? 'selected="selected"' : '', '>', soy.$$escapeHtml(opt_data.kinds[kData647].name), '</option>');
        }
      } else {
        output.append((soy.$$getMapKeys(opt_data.kinds).length != 1) ? '<option selected="selected" disabled="disabled" role=\'option\' value=\'\'>Select Analysis</option>' : '');
        var kList662 = soy.$$getMapKeys(opt_data.kinds);
        var kListLen662 = kList662.length;
        for (var kIndex662 = 0; kIndex662 < kListLen662; kIndex662++) {
          var kData662 = kList662[kIndex662];
          output.append('<option role=\'option\' value=\'', soy.$$escapeHtml(kData662), '\'>', soy.$$escapeHtml(opt_data.kinds[kData662].name), '</option>');
        }
      }
    }
...
scb_facs.analyze = function(opt_data, opt_sb) {
  ...
  var rList707 = opt_data.rows;
  var rListLen707 = rList707.length;
  for (var rIndex707 = 0; rIndex707 < rListLen707; rIndex707++) {
    var rData707 = rList707[rIndex707];
    output.append((rData707.is_valid) ? (rData707.index == 0) ? '<li role=\'listitem\' facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id="' + soy.$$escapeHtml(opt_data.assignment.id) + '" experiment_id="' + soy.$$escapeHtml(opt_data.experiment.id) + '" facs_lane_id=\'' + soy.$$escapeHtml(rData707.lane.id) + '\' lane_kind = \'' + soy.$$escapeHtml(rData707.lane.kind) + '\' conditions = \'' + soy.$$escapeHtml(rData707.lane.conditions) + '\' title=\'' + soy.$$escapeHtml(rData707.display_text) + '\'' + ((opt_data.facs.selected_lane && rData707.cell_treatment.id == opt_data.facs.selected_lane.cell_treatment_id) ? 'class=\'scb_s_facs_sample_selected\'' : '') + '>&nbsp;&nbsp;' + soy.$$escapeHtml(rData707.display_text) + '</li>' : '' : '');
  }
  ...
  if (opt_data.facs.samples_finished) {
    var rList759 = opt_data.rows;
    var rListLen759 = rList759.length;
    for (var rIndex759 = 0; rIndex759 < rListLen759; rIndex759++) {
      var rData759 = rList759[rIndex759];
      output.append((rData759.is_valid && rData759.lane.id == opt_data.facs.selected_lane.id) ? '<span class=\'scb_s_western_blot_gel_active scb_s_western_blot_gel_tab \' role=\'tab\'><div class=\'scb_s_facs_gel_tab_selected\'>' + soy.$$escapeHtml(opt_data.facs.selected_lane.kinds[opt_data.facs.selected_lane.kind].conditions[opt_data.facs.selected_lane.conditions].name) + '</div></span>' : (rData759.is_valid && rData759.cell_treatment.id == opt_data.facs.selected_lane.cell_treatment_id) ? '<div class=\'scb_s_western_blot_gel_tab\' facs_id=\'' + soy.$$escapeHtml(opt_data.facs.id) + '\' assignment_id=\'' + soy.$$escapeHtml(opt_data.assignment.id) + '\' role=\'tab\' experiment_id=\'' + soy.$$escapeHtml(opt_data.experiment.id) + '\' facs_lane_id=\'' + soy.$$escapeHtml(rData759.lane.id) + '\'>' + soy.$$escapeHtml(rData759.lane.kinds[rData759.lane.kind].conditions[rData759.lane.conditions].name) + '</div>' : '');
    }
  } else {
    ...
    if (opt_data.lane.canvas_metadata_analysis) {
      if (opt_data.lane.canvas_metadata_analysis.ranges) {
        if (opt_data.lane.canvas_metadata_analysis.ranges.length > 0) {
          ...
          var rangeList878 = opt_data.lane.canvas_metadata_analysis.ranges;
          var rangeListLen878 = rangeList878.length;
          for (var rangeIndex878 = 0; rangeIndex878 < rangeListLen878; rangeIndex878++) {
            var rangeData878 = rangeList878[rangeIndex878];
            output.append('<tr role=\'row\'><td class="', (rangeData878.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData878.display_id), '</td><td class="', (rangeData878.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData878.bisector_id), '</td><td class="', (rangeData878.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData878.from), ' - ', soy.$$escapeHtml(rangeData878.to), '</td><td class="', (rangeData878.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '">', soy.$$escapeHtml(rangeData878.percentage), '</td><td class="', (rangeData878.bisector_id == 'b') ? 'scb_s_facs_tools_analyze_bisector_border' : '', '"><img class=\'scb_f_facs_analyze_remove_point scb_s_facs_analyze_remove_point\' assignment_id=\'', soy.$$escapeHtml(opt_data.assignment.id), '\' experiment_id=\'', soy.$$escapeHtml(opt_data.experiment.id), '\' facs_id=\'', soy.$$escapeHtml(opt_data.facs.id), '\' facs_lane_id=\'', soy.$$escapeHtml(opt_data.lane.id), '\' from=\'', soy.$$escapeHtml(rangeData878.from), '\' to=\'', soy.$$escapeHtml(rangeData878.to), '\' alt="Delete" title="Delete" src="images/setup/scb_remove.png"></td></tr>');
          }
