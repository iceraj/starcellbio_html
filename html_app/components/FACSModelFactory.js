...
scb.components.FACSModelFactory = function scb_components_FACSModelFactory(model, template) {
    ...
    if (scb.utils.isDefined(model.is_ab)) {
        self.dna = function (state) {
            var t = template;
            var m = model;
            var cell_treatment = state.facs_lane.cell_treatment;
            var facs_lane = state.facs_lane;
            var shape = 'normal';
            var custom_data = [];
            _.each(m.ab_parser, function (rule) {
                console.info(rule);
                if (rule.identifier == cell_treatment.identifier
                    && rule.analysis == facs_lane.kind
                    && rule.condition == facs_lane.conditions
                    && (''+rule.treatment).toLowerCase() == (''+facs_lane.live).toLowerCase()
                    ) {
                    shape = rule.shape;
                    custom_data = rule.custom_data;
                }
            });
            state.shape = shape;
...
/////////////////////
        ...
        var options = {
            ...
            grid: {clickable: true, hoverable: true, borderWidth: 0, aboveData: true, autoHighlight: false, markings: [
                { xaxis: { from: 0, to: template.model.facs.max ? template.model.facs.max : 150 },
...
//                        {label: 'phase 4', data:[[1.8,0.011],[2.3,0.011]],lines:{fill:false}}
        ...
        if (('' + shape).toLowerCase() == 'custom') {
            var data = state.custom_data;
            state.data = {
                data: [
                    { data: data},
